/**
 * @file frame.h
 * @brief Atom-frame container and distance helper shared with the CUDA host-side driver.
 */
...
public:
     ...
     * The CUDA path is expected to pack coordinates into a device-friendly
     * structure-of-arrays layout before launching kernels.
     */
...
private:
    /**
     * Coordinates are stored on the host in an array-of-structures style
     * layout: one `(x, y, z)` record per atom.
     *
     * This is convenient for the serial, OpenMP, and MPI code paths. For CUDA,
     * we currently expect to repack these coordinates into a structure-of-
     * arrays layout (`x[]`, `y[]`, `z[]`) before copying to device memory.
     * If profiling later shows that this repacking pass is significant, this
     * host-side layout is a natural place to revisit.
     */
