#!/bin/bash
#SBATCH --job-name=cs2050_julia_strong
...
OUT_CSV="$RESULT_DIR/julia_strong.csv"
...
for t in "${THREADS_LIST[@]}"; do
    ...
    timing_line=$(run_exec "$t" "$INPUT" "$RESULT_DIR/julia_strong.csv.tmp" 10.0 --timing --no-write | awk '/Timing summary/ {print}')
