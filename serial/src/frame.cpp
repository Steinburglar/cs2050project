...
void Frame::load_from_simple_file(const std::string &path)
{
    ...
    while (std::getline(in, line))
    {
        ...
        // Try to parse either CSV or whitespace: x y z species
        double x, y, z;
        int sp;
        // replace commas with spaces for simple CSV support
        for (char &ch : line)
            if (ch == ',')
                ch = ' ';
...
// returns displacement vector between atoms, accounting for periodic boxes.
{
    ...
    // assume orthogonal box where box_[k][k] contains box length for dim k
    for (int k = 0; k < 3; ++k)
    {
        if (periodic_[k])
        {
